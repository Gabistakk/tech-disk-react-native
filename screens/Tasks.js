...
function Tasks({ navigation }) {
  ...
  return (
    <View className="h-full w-screen flex flex-col items-center pt-5">
      ...
      {loaded && data.filter(ordem => (((ordem.servico.nome.toUpperCase().includes(search.toUpperCase())) || (ordem.cliente.nome.toUpperCase().includes(search.toUpperCase()))) && (ordem.empregado.id == authData.id))).map(ordem => {
        return(
          <CardTasks key={ordem.id}
          servico={ordem.servico.nome}
          nomeDoCliente={ordem.cliente.nome}
          data={ordem.data}
